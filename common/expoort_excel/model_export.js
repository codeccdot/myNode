...
//导入Excel，xlsx格式
const  xlsxfile="C:/Users/17829/Desktop/通讯模板.xls";
router.post('/importexcelxlsx',async (ctx) => {
    ...
    async function readdata(v) {
        ...
        ctx.body=v;
        let tabname = v[0].data[1][0] 
        // 判断表里是否有该数据
        const count = exportService.getTableByName(tabname);
        ctx.type = 'json';
        if (count > 0) {
          ctx.body = {
            status: 100,
          ...
          await exportService.createTable(v);
          ctx.type = 'json';
          ctx.body = {
            status: 200,
            msg: '创建成功',
