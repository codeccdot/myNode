...
jBD.define(function (module, exports, require) {
	...
	api = {
		...
		Base64: (value, enc) => {
			...
			value = String(value);
			return enc !== false ? encode(jBD.Conver.toString(jBD.Conver.toHex(value, "utf8"), arg)) : jBD.Conver.toString(jBD.Conver.toHex(decode(value), "utf16"), arg);
		},
		/**
		 * MD5编码
		 *
		...
		SHA:    (value, opt, callback) => {
			...
			if (!jBD.has(inf, ["TEXT", "B64", "BASE64", "HEX"])) inf = "TEXT";
			if (!jBD.has(outf, ["HEX", "B64", "BASE64"])) outf = "HEX";
			if (!jBD.has(ine, ["ANSI", "UTF8", "UTF16"])) ine = "UTF8";
			if (!jBD.has(oute, ["ANSI", "UTF8", "UTF16"])) oute = "UTF8";

			if (inf === "BASE64") inf = "B64";
			if (outf === "BASE64") outf = "B64";
			if (jBD.isNull(value, true)) {
				value = "";
				inf = "TEXT";
				ine = "UTF8";
			}
		...
		CRC:    (value, type, mode) => {
			...
			if (!jBD.isArray(value)) {
				if (jBD.isString(value)) value = jBD.Conver.toHex(value, "ansi");
				else value = [];
			}
